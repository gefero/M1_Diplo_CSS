...
En esta clase vamos a ver algunas herramientas un poco más avanzadas que sirven para extender el tipo de procesamientos que vimos
...
```{r}
clasificacion.caso<- unique(base.covid$clasificacion_resumen)
clasificacion.caso
```

```{r}
for(categoria in clasificacion.caso){
...
```
Hago el mismo loop para la variable de fallecidos, para darle más sentido al ejercicio.
```{r}
categorias.fallecim<- unique(base.covid$fallecido)

for(categoria in categorias.fallecim){
...
base.covid <- base.covid %>% 
  mutate(Grupo = ifelse(edad >= 60, yes = "Riesgo",no = "Resto"))

table(base.covid$Grupo)
```

A diferencia del *ifelse*, que según el cumplimiento de la condición lógica nos permite asignar un valor, la función *if* se utilizará para correr código condicionalmente
