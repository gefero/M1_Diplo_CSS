...
comienzo_diplo <- "2021-05-04"

comienzo_diplo - today() 
```
...
```
> Practica: Creen un dataframe a partir del primer vector del ejercicio de practica anterior. Luego generen 3 nuevas columnas que detallen para cada fecha en cuestión: el numero de semana del año, el numero de dia del año, y el nombre del día  

```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
fechas.df <- data.frame(fecha.primer.vec = mdy(fechas_1)) %>% 
  mutate(semana.del.anio = week(fecha.primer.vec),
         dia.del.anio = yday(fecha.primer.vec),
         dia.nombre = wday(fecha.primer.vec,label = T))
```
...
> Practica: Filtrar un conjunto de 5 semanas consecutivas en la base de datos covid. ¿Qué dia de la semana se abren menos casos?      
...
> Practica: ¿Cuanto fue el máximo tiempo registrado en toda la base de datos entre una fecha de inicio de sintomas y el diagnóstico del caso? ¿En qué departamento y provincia ocurrió? Detallar año y mes del caso en cuestión
